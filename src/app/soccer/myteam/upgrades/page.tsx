...
import { Card, cardById, applyUpgrade } from "@/lib/soccer-myteam/cards";
import { PlayerCard } from "@/components/soccer-myteam/PlayerCard";
import { getCoins, getOwned, spendCoins } from "@/lib/store/soccer/myteam";
import {
  getLevel,
  isUpgradeable,
  upgradeCost,
  ...
  bumpLevel,
  MAX_LEVEL,
} from "@/lib/store/soccer/upgrades";
import { onCardUpgraded } from "@/lib/soccer-myteam/events";
...
export default function UpgradesPage() {
  ...
  return (
    <div className="bg-grain">
      ...
      {msg && (
        <div className="mb-4 rounded-xl border border-soccer/40 bg-soccer/10 px-4 py-2 text-sm text-soccer">
          {msg}
        </div>
      )}
    ...
    >
      ...
      >
        ...
        <div className="mt-4 space-y-2">
          ...
          <StatRow label="Pace" from={current.pace} to={next.pace} />
          <StatRow label="Shooting" from={current.shooting} to={next.shooting} />
          <StatRow label="Passing" from={current.passing} to={next.passing} />
          <StatRow label="Defending" from={current.defending} to={next.defending} />
        </div>
        ...
        {maxed ? (
          ...
          <button
            onClick={doUpgrade}
            disabled={!affordable}
            className="btn mt-5 w-full bg-soccer text-black hover:bg-soccer-gold disabled:opacity-40"
          >
