...
import { cardById, tierForCard, Card } from "@/lib/soccer-myteam/cards";
import { getOwned, getLineup } from "@/lib/store/soccer/myteam";
...
import { getRivalDeltas } from "@/lib/store/soccer/rivals";
import { onGamePlayed } from "@/lib/soccer-myteam/events";
import { buildLeaderboard, LeaderboardEntry } from "@/lib/soccer-myteam/leaderboard";
import { simulateExhibition, ExhibitionResult } from "@/lib/soccer-myteam/exhibition";
  ...
  return (
    <div className="bg-grain">
      ...
      <div className="mb-5">
        ...
        <h1 className="mt-1 text-3xl font-black">🏆 Soccer Leaderboard</h1>
        <p className="text-sm text-white/50">
          Ranked by team rating and wins. Play matches to climb.
        </p>
      </div>
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <div className="text-xs text-white/50">
              Team OVR <span className="font-bold text-soccer">{teamOvr || "—"}</span> · Record{" "}
        ...
        {teamOvr > 0 ? (
          <div className="flex gap-2">
            ...
            <button
              onClick={play}
              disabled={playing}
              className="btn bg-soccer px-6 text-black hover:bg-soccer-gold"
            >
              ▶ Play a match
            </button>
          </div>
        ) : (
          <Link href="/soccer/myteam/team" className="btn bg-soccer px-6 text-black hover:bg-soccer-gold">
            Build your squad →
          </Link>
        )}
      </div>
    ...
    >
      ...
      <div className="min-w-0 flex-1">
        <div className="truncate font-bold">
          {entry.username}
          {entry.isYou && <span className="ml-1 text-xs text-soccer">(you)</span>}
        </div>
      ...
      <div className="shrink-0 text-right">
        ...
        <div className="text-xl font-black tabular-nums text-soccer">{entry.teamOvr || "—"}</div>
      </div>
    </motion.div>
  );
}
