...
import { botSquads, userSquad, Squad } from "@/lib/soccer-myteam/squads";
import { isShared, setShared, isLiked, toggleLike, likeCount } from "@/lib/store/soccer/squads";
      ...
      <div className="card mb-6 flex flex-col gap-3 p-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <div className="text-xs text-white/50">
            {userRoster.length === 0
              ? "Build a starting XI to share it."
              : shared
        ...
        <button
          ...
          className={`btn shrink-0 ${
            shared ? "border border-white/15 text-white/80 hover:bg-white/5" : "bg-soccer text-black hover:bg-soccer-gold"
          } disabled:opacity-40`}
        >
...
function SquadCard({
  ...
  return (
    <div className={`card p-4 ${squad.isYou ? "border-soccer/50" : ""}`}>
      <div className="flex items-center justify-between">
        <div className="min-w-0">
          <div className="truncate font-bold">
            {squad.owner}
            {squad.isYou && <span className="ml-1 text-xs text-soccer">(you)</span>}
          </div>
          <div className="text-xs text-white/50">
            OVR <span className="font-bold text-soccer">{squad.ovr}</span> ·{" "}
