...
import { Card, cardById } from "@/lib/myteam/cards";
...
import { getRivalDeltas, applyRivalResult } from "@/lib/store/rivals";
...
export default function RivalsPage() {
  ...
  const refresh = useCallback(() => {
    ...
    const rec = getRecord();
    setUser({ name: username, ovr, roster: starters, wins: rec.wins, losses: rec.losses });
    ...
    <div className="bg-grain">
      <div className="mb-5">
        ...
        <p className="text-sm text-white/50">
          Challenge GMs near your rating. Every game counts on the leaderboard.
        </p>
      </div>
      ...
      <div className="card mb-6 flex items-center justify-between p-4">
        <div>
          ...
          <div className="text-xs text-white/50">
            Team OVR <span className="font-bold text-nba">{user.ovr || "—"}</span> · {user.wins}-{user.losses}
          </div>
        </div>
        <Link href="/myteam/leaderboard" className="text-sm text-white/50 hover:text-white">
          Leaderboard →
        </Link>
      </div>
      ...
      {user.ovr <= 0 ? (
        <div className="card p-10 text-center text-sm text-white/45">
          Build a starting five before you can challenge rivals.{" "}
          <Link href="/myteam/team" className="text-nba underline">
            Team Builder →
          </Link>
        </div>
      ) : (
...
function RivalCard({
  ...
  const best = rival.bestCardId ? cardById(rival.bestCardId) : null;
  return (
    <div className="card flex items-center gap-4 p-4">
      ...
      <div className="min-w-0 flex-1">
        ...
        <div className="text-xs text-white/50">
          OVR <span className="font-bold text-nba">{rival.teamOvr}</span> · {rival.wins}-{rival.losses}
        </div>
    ...
    >
      <div className="mx-auto min-h-full max-w-3xl p-5">
        ...
        <div className="mb-5 flex items-center justify-center gap-4 pt-4 text-center sm:gap-8">
          <SideHead name={user.name} ovr={user.ovr} accent="text-nba" />
          <motion.div
        ...
        {phase === "result" && result ? (
          ...
          >
            ...
            <div className="my-2 text-6xl font-black tabular-nums">
              <span className={result.userWon ? "text-nba" : "text-white/70"}>{result.userScore}</span>
            ...
            <div className="mx-auto mt-6 flex max-w-sm gap-3">
              ...
              <Link href="/myteam/leaderboard" className="btn flex-1 bg-white/10 text-white hover:bg-white/15">
                Leaderboard
              </Link>
              <button onClick={onClose} className="btn flex-1 bg-nba text-black hover:bg-nba-gold">
                Done
              </button>
            </div>
          </motion.div>
          ...
          <div className="py-16 text-center">
            ...
            <p className="mt-4 text-sm uppercase tracking-[0.3em] text-white/40">Tip-off…</p>
          </div>
        ) : (
          <>
            {/* scout both rosters before accepting */}
            <RosterStrip title="Your starting five" cards={user.roster} accent="border-nba/40" />
            <div className="my-4 text-center text-xs uppercase tracking-widest text-red-400/80">
              ⚡ Scouting report ⚡
            </div>
            <RosterStrip title={`${rival.username}'s starting five`} cards={rival.roster} accent="border-red-500/40" />
