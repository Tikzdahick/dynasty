            ...
            <div key={c.id} className="card p-4">
              <div className="flex items-center justify-between gap-3">
                ...
                {c.claimed ? (
                  ...
                  <button
                    onClick={() => onClaim(c)}
                    className="btn shrink-0 bg-amber-400 px-4 py-1.5 text-sm text-black hover:bg-amber-300"
                  >
                    Claim
                  </button>
                ) : (
              ...
              <div className="mt-3 h-2 overflow-hidden rounded-full bg-white/10">
                <motion.div
                  className={`h-full rounded-full ${c.complete ? "bg-emerald-400" : "bg-nba"}`}
                  initial={{ width: 0 }}
                  animate={{ width: `${pct}%` }}
                />
              </div>
