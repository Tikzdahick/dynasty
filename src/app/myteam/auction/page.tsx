...
import { getCoins, getOwned, OwnedCard } from "@/lib/store/myteam";
import {
  Listing,
  getListings,
  ensureSeeded,
  ...
  myBids,
  myListings,
} from "@/lib/store/auction";
import { onAuctionSold, onAuctionOutbid } from "@/lib/myteam/events";
...
export default function AuctionPage() {
  ...
  // surface market notes as transient toasts AND persistent notifications
    ...
    <div className="bg-grain">
      ...
      <div className="mb-5 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex items-center gap-3 self-start">
          <button onClick={() => setSelling(true)} className="btn bg-nba text-black hover:bg-nba-gold">
            + Sell a card
          </button>
        ...
        {owned.length === 0 ? (
          ...
          <>
            ...
            <input
              ...
              inputMode="numeric"
              className="mt-1 w-full rounded-xl border border-white/10 bg-panel px-3 py-2.5 text-sm outline-none focus:border-nba/60"
            />
              ...
              {[1, 6, 12, 24].map((h) => (
                <button
                  ...
                  className={`flex-1 rounded-xl border py-2 text-sm font-semibold transition ${
                    hours === h ? "border-nba bg-nba/10 text-nba" : "border-white/10 text-white/60 hover:border-white/20"
                  }`}
                >
                  {h}h
                </button>
            ...
            <button onClick={submit} className="btn mt-4 w-full bg-nba text-black hover:bg-nba-gold">
              List for auction
            </button>
          </>
        )}
