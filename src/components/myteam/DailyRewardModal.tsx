  ...
  return (
    ...
    >
      <motion.div
        ...
        className="relative w-full max-w-xl overflow-hidden rounded-3xl border border-amber-300/25 bg-ink/95 p-6"
      >
        {/* ambient glow */}
        <div className="pointer-events-none absolute -top-24 left-1/2 h-48 w-48 -translate-x-1/2 rounded-full bg-amber-400/20 blur-[80px]" />
          ...
          {phase === "track" ? (
            <motion.div key="track" exit={{ opacity: 0 }}>
              <div className="text-center">
                ...
                <div className="mt-2 inline-flex items-center gap-1.5 rounded-full bg-amber-400/15 px-3 py-1 text-sm font-bold text-amber-300">
            ...
            >
              ...
              <button
                onClick={onClose}
                className="btn mt-6 w-full bg-amber-400 text-black hover:bg-amber-300"
              >
                Awesome →
              </button>
            </motion.div>
  ...
  return (
    <motion.div
      ...
      className={`relative flex flex-col items-center justify-center rounded-xl border p-2 text-center ${
        active
          ? "border-amber-300 bg-amber-400/15 shadow-[0_0_24px_-6px_rgba(251,191,36,0.7)]"
          : done
    ...
    >
      ...
      {active && (
        <span className="absolute -top-2 rounded-full bg-amber-400 px-1.5 text-[8px] font-black text-black">
          TODAY
        </span>
      )}
    </motion.div>
...
function RewardReveal({ result }: { result: GrantResult }) {
  if (result.cards.length > 0) {
    const best = result.cards.reduce((a, c) =>
      c.overall > a.overall ? c : a
    );
    return (
      <div className="mt-3">
        <p className="text-lg font-bold">
          You pulled{" "}
          <span className={tierForCard(best).text}>{best.name}</span>!
        </p>
  ...
  return (
    ...
    >
      ...
      <div className="mt-2 text-4xl font-black tabular-nums text-amber-300">
