// Soccer squad sharing — likes + the user's public-share flag. Browsable squads
// are generated from the rival pool (see lib/soccer-myteam/squads.ts).
import { mulberry32, seedFromString } from "@/lib/rng";

const LIKES_KEY = "dynasty.sc.squads.likes"; // squad ids the user has liked
const SHARE_KEY = "dynasty.sc.squads.shared"; // "1" if the user shared their squad
...
export function likeCount(id: string): number {
  const base = Math.floor(mulberry32(seedFromString(`sc:likes:${id}`))() * 240) + 5;
