// Soccer MyTeam — Auction House marketplace (simulated live market for
// guest/local mode). Namespaced separately from the NBA auction house.
import {
  Card,
  CARDS_BY_RARITY,
  RARITY_ORDER,
...
const LISTINGS_KEY = "dynasty.sc.auction.listings";
const ESCROW_KEY = "dynasty.sc.auction.escrow";
const SEED_KEY = "dynasty.sc.auction.seeded";
...
const BOT_NAMES = [
  ...
  "ParkTheBus", "Catenaccio", "FalseNine", "CornerTaken", "WingBackWizard",
...
export function tickMarket(): string[] {
  ...
  for (const l of listings) {
    ...
    const willBid =
      Math.random() < urgency * rarityPull && l.highBidder !== null
        ? true
