// MyTeam — pack definitions and the weighted pull engine. Higher tiers cost
// more and shift the odds toward rare cards; a `guarantee` floor ensures the
// premium packs always deliver at least one card of that rarity or better.
import {
  Card,
  Rarity,
  RARITY_ORDER,
...
export interface PackDef {
  ...
  size: number;
  blurb: string;
  accent: string; // tailwind text token for theming
  weights: Partial<Record<Rarity, number>>;
  guarantee?: Rarity; // at least one card of this rarity or higher
}
...
export const PACKS: PackDef[] = [
  {
    ...
    price: 250,
    size: 3,
    blurb: "Three cards. Mostly role players — but a star can slip in.",
  ...
  },
  {
    ...
    price: 2500,
    size: 5,
    blurb: "Five cards, a guaranteed Diamond, and the best shot at a Dynasty legend.",
...
function drawCard(rarity: Rarity, rng: () => number): Card {
  ...
  // fall back upward if we ran off the bottom
  for (const r of RARITY_ORDER) if (CARDS_BY_RARITY[r].length) return CARDS_BY_RARITY[r][0];
  throw new Error("No cards available");
}

// default single-pull odds, used by rewards that grant one card
...
export function openPack(pack: PackDef, rng: () => number = Math.random): Card[] {
  ...
  for (let i = 0; i < pack.size; i++) {
    ...
    let rarity: Rarity;
    if (needGuarantee) {
      // restrict the roll to the guaranteed tier and above
