// Daily & Weekly Challenges. Daily set rotates every 24h, weekly every 7 days;
// both are picked deterministically from pools by date so they're stable within
// the period. Progress is driven by event metrics (see events.ts) and claims
// pay out via the shared reward + XP systems.
...
const DAILY_POOL: ChallengeDef[] = [
  { id: "d_win2", desc: "Win 2 games", metric: "gamesWon", goal: 2, xp: 60, reward: { kind: "coins", amount: 200, emoji: "🪙", label: "200 Coins" } },
  { id: "d_play3", desc: "Play 3 games", metric: "gamesPlayed", goal: 3, xp: 50, reward: { kind: "coins", amount: 150, emoji: "🪙", label: "150 Coins" } },
...
const WEEKLY_POOL: ChallengeDef[] = [
  { id: "w_win10", desc: "Win 10 games this week", metric: "gamesWon", goal: 10, xp: 250, reward: { kind: "card", minRarity: "Gold", emoji: "⭐", label: "Gold+ Card" } },
...
export function activeDailyDefs(): ChallengeDef[] {
  return pick(DAILY_POOL, `daily:${dayKey()}`, 3);
}
export function activeWeeklyDefs(): ChallengeDef[] {
  return pick(WEEKLY_POOL, `weekly:${weekKey()}`, 2);
}
...
const DAILY_KEY = "dynasty.challenges.daily";
const WEEKLY_KEY = "dynasty.challenges.weekly";
